...
The backend is split into 9 packages with strict downward dependencies (no cycles):
...
- `internal/logbuf/` — leaf package: log buffer + slog handler infrastructure (`LogBuffer`, `LogEntry`, `LogFilter`, `BufferHandler`, `NewMultiHandler` (returns unexported `multiHandler`), `SlogWriter`, package logger via `Logger`/`SetLogger`).
...
- `internal/org/` — depends on apitypes, snapshot, pod: the `OrgService` domain. Constructor `org.New(snapshot.Store)`. Files include service.go (state queries, `CaptureState`/`ApplyState`, snapshot orchestration via `SaveSnapshot`/`LoadSnapshot`/`ExportSnapshot` — the only place reserved-name routing for Working/Original lives), people.go (`Move`, `Update`, `Add`, `AddParent`, `Delete`, `Restore`, `EmptyBin`, `Reorder`), pods.go (`ListPods`, `UpdatePod`, `CreatePod`, `GetPodExportData`), settings.go (`GetSettings`, `UpdateSettings`), import.go (`Upload`, `ConfirmMapping`, `UploadZip`), errors.go (typed errors `ValidationError`/`NotFoundError`/`ConflictError` plus `ServiceError` HTTP mapper), validate.go (input validation: `validateNodeUpdate`, `validateManagerChange`, `wouldCreateCycle`, `validateSettings`), types.go (response DTOs `MoveResult`/`MutationResult`/`OrgData`/`UploadResponse`), constants.go, convert.go, export.go, infer.go, zipimport.go. OrgService never calls `s.snap.*` while holding `s.mu` — the only remaining lock-order discipline (auditable via grep, since the snapshot side cannot violate).
- `internal/httpapi/` — depends on all above: HTTP transport layer. router.go (`NewRouter`), handlers.go (shared helpers: `jsonHandlerCtx` generic, `readUploadedFile`, `writeJSON`/`writeError`, `exportByFormat`, `limitBody`, `sanitizeFilename`, `writeFileResponse`), handlers_org.go (people + org-state mutations + org export), handlers_pod.go (`handleListPods`/`handleUpdatePod`/`handleCreatePod`/`handleExportPodsSidecar`), handlers_snapshot.go (`handleListSnapshots`/`handleSaveSnapshot`/`handleLoadSnapshot`/`handleDeleteSnapshot`/`handleExportSnapshot`), handlers_import.go (`handleUpload`/`handleUploadZip`/`handleConfirmMapping`), handlers_settings.go (`handleGetSettings`/`handleUpdateSettings`/`handleExportSettingsSidecar`), csrf.go (`csrfProtect`, `sameOriginOrAbsent`), middleware.go (`LoggingMiddleware` + `responseCapture` + log endpoints), services.go (`Services` struct + 6 role interfaces `NodeService`/`OrgStateService`/`SnapshotOps`/`ImportService`/`PodService`/`SettingsService` + `NewServices` ctor + compile-time assertions), responses.go (`WorkingResponse`, `AddResponse`, `MutationResponse`, `RecycledResponse`, `HealthResponse`, `ConfigResponse`), autosave_handlers.go.
...
- `web/src/store/ViewDataContext.tsx` — Derived view-data provider with three split contexts: exports `usePeople()` (filtered/sorted people), `useChanges()` (diff annotations), `useActions()` (view-scoped action callbacks)
...
- `web/src/store/useOrgMutations.ts` — Mutation wrappers (Move/Update/Add/Delete/Restore/Reorder) that call the API client and apply optimistic updates
- `web/src/store/useAutosaveActions.ts` — Autosave debounce + restore wiring
- `web/src/store/useUndoRedoActions.ts` — Undo/redo stack actions; pairs with `hooks/useUndoRedo.ts`
- `web/src/store/useInitialBootstrap.ts` — First-load fetch of working/snapshots/settings
- `web/src/store/useInteractionState.ts` — Drag/lasso/hover transient state shared across views
- `web/src/store/optimistic.ts` — Pure helpers for optimistic state transitions
- `web/src/views/ChartShell.tsx` — Shared chart frame parameterized by `computeEdges`/`computeLayout`/`renderLayoutNode`; provides `DndContext`, lasso, drag overlay, popovers. ColumnView and ManagerView render through it.
...
- `web/src/views/chartSelectors.ts` — Memoized selectors for chart-level derived data
...
- `web/src/views/TableView.tsx` + `TableHeader.tsx` / `TableRow.tsx` / `TableCell.tsx` / `TableFilterDropdown.tsx` / `tableColumns.ts` — Tabular data view with sortable columns and per-column filters
- `web/src/views/columnEdges.ts` — SVG edge geometry for ColumnView connector lines
...
- `web/src/components/` — UI primitives. Notable: `BaseNode.tsx` (shared card chrome), `OrgNodeCard.tsx` (person card with status styling + hover actions), `GroupHeaderNode.tsx`, `NodeForm.tsx` / `NodeEditSidebar.tsx` / `DetailSidebar.tsx` / `BatchEditSidebar.tsx` / `SidebarShell.tsx` (edit forms), `ColumnMappingModal.tsx` (CSV header mapping), `Toolbar.tsx`, `SearchBar.tsx`, `Breadcrumbs.tsx`, `FiltersDropdown.tsx` / `EmploymentTypeFilter.tsx`, `RecycleBinButton.tsx` / `RecycleBinDrawer.tsx`, `SnapshotsDropdown.tsx`, `ExportDropdown.tsx`, `SettingsModal.tsx`, `AutosaveBanner.tsx`, `LogPanel.tsx`, `VimCheatSheet.tsx`, `ErrorBoundary.tsx`, `UploadPrompt.tsx`, popovers (`AddParentPopover`, `DeleteConfirmPopover`, `ManagerInfoPopover`), `UnparentedBar.tsx`, `PrivateToggle.tsx`, `PodSidebar.tsx`, `NodeActions.tsx`.
- `web/src/hooks/` — Reusable hooks (non-exhaustive): data hooks (`useOrgDiff`, `useIsManager`, `useOrgMetrics`, `useFilteredPeople`, `useSortedPeople`, `useHeadSubtree`, `useSaveStatus`); interaction (`useDragDrop`, `useLassoSelect`, `useSpatialNav`, `useOutsideClick`, `useUnifiedEscape`); chart (`useChartLayout`, `useNodeProps`); features (`useAutosave`, `useExport`, `useSnapshotExport`, `useUndoRedo` + `useUndoRedoKeys`, `useTour`, `useTheme`, `useDeepLink`, `useLogging`); vim (`useVimMode`, `useVimNav`).
