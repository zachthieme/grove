...
describe('computeOrgMetrics — additional branches', () => {
  it('groups by pod name when pod is set', () => {
    const mgr = makeNode({ id: 'mgr' })
    const ic = makeNode({ id: 'ic1', managerId: 'mgr', pod: 'Alpha Pod', status: 'Active' })
  ...
  it('uses "Unknown" discipline for active person with empty discipline', () => {
    const mgr = makeNode({ id: 'mgr' })
    const ic = makeNode({ id: 'ic1', managerId: 'mgr', discipline: '', status: 'Active' })
    const metrics = computeOrgMetrics('mgr', [mgr, ic])
  ...
  it('counts Backfill as recruiting', () => {
    const mgr = makeNode({ id: 'mgr' })
    const ic = makeNode({ id: 'ic1', managerId: 'mgr', status: 'Backfill' })
    const metrics = computeOrgMetrics('mgr', [mgr, ic])
  ...
  it('counts Transfer Out as transfers', () => {
    const mgr = makeNode({ id: 'mgr' })
    const ic = makeNode({ id: 'ic1', managerId: 'mgr', status: 'Transfer Out' })
    const metrics = computeOrgMetrics('mgr', [mgr, ic])
  ...
  it('uses "Unassigned" for people with no team and no pod', () => {
    const mgr = makeNode({ id: 'mgr' })
    const ic = makeNode({ id: 'ic1', managerId: 'mgr', team: '', pod: '', status: 'Active' })
    const metrics = computeOrgMetrics('mgr', [mgr, ic])
  ...
  it('handles deeply nested hierarchy', () => {
    const top = makeNode({ id: 'top' })
    const mid = makeNode({ id: 'mid', managerId: 'top' })
    const bottom = makeNode({ id: 'bot', managerId: 'mid' })
  ...
  it('sorts byTeamPod by count descending', () => {
    const mgr = makeNode({ id: 'mgr' })
    const ic1 = makeNode({ id: 'ic1', managerId: 'mgr', team: 'Small' })
    const ic2 = makeNode({ id: 'ic2', managerId: 'mgr', team: 'Big' })
    const ic3 = makeNode({ id: 'ic3', managerId: 'mgr', team: 'Big' })
